...
<script>
import bin_team from '/bin_data.json';
import kcr_team from '/kcr_data.json';
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    fetch('http://localhost:5000/start_match', {
      ...
      body: JSON.stringify({ match_type: '3v3' })
    })
  ...
  mounted() {
      ...
      let ballIcon = L.icon({
                  iconUrl: '/assets/icons/3v3/ball.png',
                  iconSize: [40, 40],
                  popupAnchor: [0, -20]
              });
      ...
      function addPlayerMarkers(team, teamName) {
          team.forEach(player => {
              let playerIcon = L.icon({
                  iconUrl: `/assets/icons/3v3/${teamName}/png/BL_Icon_${player.first_name}_${player.last_name}.png`,
                  iconSize: iconSize,
      ...
      addPlayerMarkers(bin_team, 'BIN');
      addPlayerMarkers(kcr_team, 'KCR');
      ...
      this.socket.onmessage = function(event) {
          ...
          coordinates.forEach(coord => {
              if (coord.first_name === 'Football' && coord.last_name === 'Ball') {
                  ...
                  // Check player is in bin_team or kcr_team
                  if(bin_team.some(player => player.first_name === coord.first_name && player.last_name === coord.last_name) ||
                     kcr_team.some(player => player.first_name === coord.first_name && player.last_name === coord.last_name)) {
